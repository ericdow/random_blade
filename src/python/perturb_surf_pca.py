...
def perturb(rpath, wpath, pca_path, Z_path, n_pca):

    # inputs
    # Z_path : where to write out the sample normal 
    # n_pca  : where to truncate the PCA expansion

    cdim, sdim, x, y, z = read_blade.read_coords(rpath)

    # generate and write out the Z for this mesh
    random.seed() 
    Z = random.randn(n_pca)    # normal distribution vector
    f = open(Z_path,'w')
...
# quick test
rpath = 'tmp/blade_surf.dat'
wpath = 'tmp/blade_surf_mod.dat'
pca_path = '/home/ericdow/code/random_blade/input/rotor37_coarse/r37_pca/'
Z_path = 'tmp/Z_test.dat'
n_pca = 6
perturb(rpath, wpath, pca_path, Z_path, n_pca)
